...
public class FrontServlet extends HttpServlet {
    ...
    private Method findMethod(Class<?> pageClass, Route route) {
        Method method = null;
    ...
    private void invokeMethod(Method method, Object page, HttpServletRequest request, Map<String, Object> view)
            throws ServletException {
        try {
            method.setAccessible(true);
            method.invoke(page, request, view);
        } catch (IllegalAccessException e) {
            throw new ServletException("Unable to run action [method=" + method + "].", e);
    ...
    private void process(Route route,
        ...
        try {
            Method beforeMethod = findMethod(pageClass, Route.newRouteBefore(route));
            if (beforeMethod != null) {
                invokeMethod(beforeMethod, page, request, view);
            }
            ...
            invokeMethod(method, page, request, view);

            Method afterMethod = findMethod(pageClass, Route.newRouteAfter(route));
            if (afterMethod != null) {
                invokeMethod(afterMethod, page, request, view);
            }
    ...
    private static final class Route {
        ...
        private static final String BEFORE_ACTION = "before";
        private static final String AFTER_ACTION = "after";
        ...
        private static Route newRouteBefore(Route route) {
            return new Route(route.getClassName(), BEFORE_ACTION);
        }

        private static Route newRouteAfter(Route route) {
            return new Route(route.getClassName(), AFTER_ACTION);
        }
