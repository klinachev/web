...
class HttpServer {
    ...
    private static String getFileETag(File file) {
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        String requestsUri = URLDecoder.decode(request.getUri(), "UTF-8");
        File file = new File(root, requestsUri);
        if (file.isDirectory()) {
            file = new File(file, "/index.html");
        }
        if (file.isFile()) {
            String eTagValue = getFileETag(file);
            response.setHeader("ETag", eTagValue);
            if (eTagValue.equals(request.getHeader("If-None-Match"))) {
    ...
    private byte[] readInputStream(InputStream inputStream, boolean breakOnCrLf) throws IOException {
        ...
        // System.out.println(bytes.size() + " end");
    ...
    private Request readRequest(Socket socket) throws IOException {
        ...
        String a = new String(readInputStream(inputStream, true));
